...
# 定义全局Cookie变量
login_page = 'http://login.ald.idm.oclc.org/login?auth=III&url=r0$https://infoweb.newsbank.com/apps/news/easy-search?p=AWNB'

# 账号配置 - 每个账号对应一个驱动和线程
account_configs = [
    {
        'username': '21393012106328',
        'password': '1234',
        'driver': None,
        'thread_id': 1
    },
    {
        'username': '21393012110841',
        'password': '1234',
        'driver': None,
        'thread_id': 2
    },
    {
        'username': '21393012110858',
        'password': '1234',
        'driver': None,
        'thread_id': 3
    },
    {
        'username': '21393012110866',
        'password': '1234',
        'driver': None,
        'thread_id': 4
    },
    # {
    #     'username': '21393012110874',
    #     'password': '1234',
    #     'driver': None,
    #     'thread_id': 5
    # },
    {
        'username': '21393012110882',
        'password': '1234',
        'driver': None,
        'thread_id': 6
    },
...
domain_url = 'https://infoweb-newsbank-com.ald.idm.oclc.org'
...
RETRY_CONFIG = {
    ...
    'max_retry_delay': 20,               # 最大重试延迟（秒）
    'exception_retry_delay_multiplier': 3,  # 异常重试延迟倍数（增加到3倍）
    'enable_browser_recreate': True,     # 是否启用浏览器重建
    'chrome_crash_retry_delay': 10,      # Chrome崩溃后重试延迟（秒）
    'connection_timeout_retry': True,     # 连接超时重试
    'page_load_timeout': 30,             # 页面加载超时（秒）
    'element_wait_timeout': 60           # 元素等待超时（秒）
}
...
def get_cookie_for_thread(thread_id, logger_thread, username, password):
    ...
    try:
        ...
        logger_thread.info(f"[线程{thread_id}|账号{username}] ✅ 登录成功（用户名+密码模式）")
        return True
...
def crawl_data(file_name, pubname, date_range, climate, policy, uncertainty, logger_thread):
    ...
    for filter_data in filter_data_set:
        ...
        if len(parts) == 3:
            ...
            date_year = [part for part in date.split(' ') if part.isdigit()][-1]
