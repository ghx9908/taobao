...
def get_cookie_for_thread(thread_id, logger_thread, username, password=None):
    """为指定线程获取 Cookie - 适配 DC 公共图书馆登录（仅 Library Card / DC One Card，无密码）"""
    ...
    try:
        ...
        # 2. 点击 Log In 按钮（<button>Log In</button>，无密码字段）
        login_button = wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "form[action='/login'] button"))
        )
        ...
        # 3. 等待跳转完成（跳回 newsbank，标题包含 Access World News）
        wait.until(EC.title_contains("Access World News"))
        logger_thread.info(f"[线程{thread_id}|账号{username}] ✅ 登录成功（DC 卡号登录）")
        return True
